...
--batch-split 2 \
--nbatches 1000 \
--checkpoint checkpoints/finetuning/sst5/medium/ellattention-smoe-m.pt
--pretrained_weight checkpoints/pretraining/enwik8/ellattention-smoe-m.pt \
--distributed \
"

echo "Training ..."
CUDA_VISIBLE_DEVICES='0' python -m torch.distributed.launch --master_port 1903 --nproc_per_node=1 --use_env finetune_train.py $args
